...
import string
import base64
from lexer2 import Token, Lexer
...
def NO_MORE_CHARS():
    if (Input.EOF_Read and Input.Next >= Input.LEndBuf):
        return True
    return False
...
def read_funct(fd, starting_at, need):
    ...
    try:
        ...
        #Input.StartBuf = array.array('B', [96 for x in range(Input.BUFSIZE)]) # StartBuf = ['' for x in range(BUFSIZE)]
        #Input.MVStartBuf = memoryview(Input.StartBuf)
        fd.readinto(Input.MVStartBuf[starting_at:])
        #Input.MVStartBuf = memoryview(Input.StartBuf)
...
if __name__ == '__main__':
    ...
    #     # ignore whitespace
        ...
        # identifier token
            ...
            while c in string.ascii_letters:
                match += c
                c = chr(ii_advance())
                i += 1
                print(f'c : {i} - {c}')
